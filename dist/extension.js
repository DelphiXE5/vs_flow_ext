...
const previewManager_1 = __webpack_require__(2);
const utilities = __webpack_require__(14);
const previewCommand = "flowfile.showPreviewToSide";
// Extension interfaces.
function activate(context) {
    return __awaiter(this, void 0, void 0, function* () {
        const previewHtml = yield utilities.readFileAsync(context.asAbsolutePath("resources/preview.html"), "utf8");
        const panelSnippet = yield utilities.readFileAsync(context.asAbsolutePath("resources/panel.html"), "utf8");
        const previewManager = new previewManager_1.PreviewManager(previewHtml, panelSnippet);
...
class PreviewManager {
    ...
    showPreviewToSide(editor) {
        return __awaiter(this, void 0, void 0, function* () {
            const document = editor.document;
            const context = this.previewContexts.get(document);
            if (context === undefined) {
                this.previewContexts.set(document, yield this.createPreview(document, vscode.ViewColumn.Beside));
            }
            else {
    ...
    createPreview(document, column) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            webviewPanel.webview.html = this.getWebviewContent(document).replace(/\{base-url\}/g, uriToVscodeResource(documentDirUri));
    ...
    getWebviewContent(document) {
        let previewContent = this.template;
        let panelSnippet = this.panel;
        previewContent = previewContent.replace(/\{heading\}/g, document.fileName.replace(/^.*[\\\/]/, "")); //Add head title
        try {
            var jsonObj = JSON.parse(Parser.xml2json(document.getText(), options));
            let count = 1;
            for (let scenario of jsonObj.elements[0].elements[0].elements) {
                panelSnippet = this.panel;
                panelSnippet = panelSnippet.replace(/\{scenario_num\}/g, "Szenario " + count + ": "); //Add scenario count
                panelSnippet = panelSnippet.replace(/\{scenario_title\}/g, scenario.elements.filter((obj) => {
                    return obj.name === "title";
                })[0].elements[0].text); //Add scenario title
                panelSnippet = panelSnippet.replace(/\{scenario_description\}/g, Parser.js2xml(scenario.elements.filter((obj) => {
                    return obj.name === "description";
                })[0])); //add scenario description (with xml)
                panelSnippet = panelSnippet.replace(/\<sentence \/\>/g, Parser.js2xml(scenario.elements.filter((obj) => {
                    return obj.name === "test";
                })[0]));
                count++;
                previewContent = previewContent.replace(/\<panel \/\>/g, panelSnippet);
            }
            //Parse feature to html
            previewContent = previewContent.replace(/\{qtext\}/g, JSON.stringify(jsonObj.elements[0].elements[0].elements));
        }
        catch (e) {
            previewContent = previewContent.replace(/\<error \/\>/g, '<div id="error" class="panel">' + e + "</div>");
        }
...
var xml2js = __webpack_require__(5);
var xml2json = __webpack_require__(11);
var js2xml = __webpack_require__(12);
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.runChildProcess = exports.writeFileAsync = exports.readFileAsync = void 0;
