    ...
    private getWebviewContent(document: vscode.TextDocument): string {
        ...
        try {
            ...
            for (let scenario of jsonObj.elements[0].elements[0].elements){
                ...
                panelSnippet = panelSnippet.replace(/\{scenario_title\}/g, scenario.elements.filter((obj: any) => {return obj.name == "title"})[0].elements[0].text); //Add scenario title
                panelSnippet = panelSnippet.replace(/\{scenario_description\}/g, Parser.js2xml(scenario.elements.filter((obj: any) => {return obj.name == "description"})[0])); //add scenario description (with xml)
                panelSnippet = panelSnippet.replace(/\<sentence \/\>/g, Parser.js2xml(scenario.elements.filter((obj: any) => {return obj.name == "test"})[0]));
                count ++;
